...
SELECT 
    ...
    IFNULL(
        (
            SELECT CONCAT('$', FORMAT(SUM(ih.Holdings_Value), 2))
            FROM Investment_Holding ih
            WHERE ih.Account_ID = ia.Account_ID
        ), '$0.00') AS Total_Holdings_Value,
    IFNULL(
        (
            SELECT GROUP_CONCAT(CONCAT(ih.Investment_Symbol, ': $', FORMAT(ih.Holdings_Value, 2)) SEPARATOR ', ')
            FROM Investment_Holding ih
            WHERE ih.Account_ID = ia.Account_ID
        ), 'No Holdings') AS Holdings_Overview,
    IFNULL(
        (
            SELECT GROUP_CONCAT(CONCAT(iaa.Action_Type, ': $', FORMAT(iaa.Quantity * iaa.Price_Per_Share, 2)) SEPARATOR ', ')
            FROM Investment_Asset iaa
            WHERE iaa.Account_ID = ia.Account_ID
        ), 'No Transactions') AS Recent_Transactions,
    IFNULL(
        (
            SELECT GROUP_CONCAT(CONCAT(ih.Investment_Symbol, ': $', FORMAT(ih.Unrealized_Gain_Or_Loss, 2)) SEPARATOR ', ')
            FROM Investment_Holding ih
            WHERE ih.Account_ID = ia.Account_ID
...
ORDER BY ia.Total_Investment_Value DESC;
