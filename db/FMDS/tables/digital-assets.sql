...
SELECT 
    ...
    IFNULL(
        (
            SELECT CONCAT('$', FORMAT(SUM(dh.Holdings_Value), 2))
            FROM Digital_Asset_Holding dh
            WHERE dh.Account_ID = daa.Account_ID
        ), '$0.00') AS Total_Holdings_Value,
    IFNULL(
        (
            SELECT GROUP_CONCAT(CONCAT(dh.Digital_Asset_Symbol, ': $', FORMAT(dh.Holdings_Value, 2)) SEPARATOR ', ')
            FROM Digital_Asset_Holding dh
            WHERE dh.Account_ID = daa.Account_ID
        ), 'No Holdings') AS Holdings_Overview,
    IFNULL(
        (
            SELECT GROUP_CONCAT(CONCAT(da.Action_Type, ': $', FORMAT(da.Quantity * da.Price_Per_Token, 2)) SEPARATOR ', ')
            FROM Digital_Asset da
            WHERE da.Account_ID = daa.Account_ID
        ), 'No Transactions') AS Asset_Transactions,
    IFNULL(
        (
            SELECT GROUP_CONCAT(CONCAT(dh.Digital_Asset_Symbol, ': $', FORMAT(dh.Unrealized_Gain_Or_Loss, 2)) SEPARATOR ', ')
            FROM Digital_Asset_Holding dh
            WHERE dh.Account_ID = daa.Account_ID
        ), 'No Unrealized Gains/Losses') AS Unrealized_Gains_Losses,
...
ORDER BY daa.Total_Digital_Assets_Value DESC;
